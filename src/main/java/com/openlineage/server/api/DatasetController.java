...
public class DatasetController {
        ...
        public com.openlineage.server.api.models.DatasetResponse.DatasetsResponse listDatasets(
                ...
                org.springframework.data.domain.Page<DatasetDocument> page = repository.findByIdNamespace(namespace,
                                pageRequest);

                // Batch-load output facets to check column lineage presence
                List<MarquezId> datasetIds = page.getContent().stream()
                                .map(DatasetDocument::getId)
                                .collect(java.util.stream.Collectors.toList());
                java.util.Set<MarquezId> hasColumnLineageSet = outputFacetRepository.findAllById(datasetIds).stream()
                                .filter(f -> f.getFacets() != null && f.getFacets().containsKey("columnLineage"))
                                .map(OutputDatasetFacetDocument::getDatasetId)
                                .collect(java.util.stream.Collectors.toSet());

                List<com.openlineage.server.api.models.DatasetResponse> datasets = page.getContent()
                                .stream()
                                .map(doc -> datasetMapper.toResponse(doc, hasColumnLineageSet.contains(doc.getId())))
                                .collect(java.util.stream.Collectors.toList());
                return new com.openlineage.server.api.models.DatasetResponse.DatasetsResponse(datasets,
                                ...
                                .map(doc -> {
                                        com.openlineage.server.domain.Dataset ds = doc.getEvent().outputs().stream()
                                                        .filter(d -> d.namespace().equals(namespace)
                                                                        && d.name().equals(datasetName))
                                                        .findFirst()
                                                        .orElseThrow(() -> new IllegalStateException(
                                                                        "Dataset not found in event outputs"));
                ...
                com.openlineage.server.api.models.RunResponse runResponse = new com.openlineage.server.api.models.RunResponse(
                                ...
                                doc.getEvent().inputs() != null ? doc.getEvent().inputs().stream()
                                                .map(d -> new com.openlineage.server.api.models.DatasetResponse(
                                                                new com.openlineage.server.api.models.DatasetResponse.DatasetId(
                                                                                d.namespace(),
                                                                                d.name()),
                                                                "DB_TABLE", d.name(), d.name(), null, null,
                                                                d.namespace(), d.namespace(),
                                                                java.util.Collections.emptyList(),
                                                                java.util.Collections.emptySet(), null, null,
                                                                java.util.Collections.emptyList(),
                                ...
                                new com.openlineage.server.api.models.RunResponse.JobVersion(
                                                doc.getEvent().job().namespace(),
        ...
        private java.util.List<Object> mapFields(com.openlineage.server.domain.Dataset ds) {
                if (ds.facets() != null && ds.facets().containsKey("schema")) {
                        ...
                        if (schemaFacet instanceof com.openlineage.server.domain.SchemaDatasetFacet) {
                                return ((com.openlineage.server.domain.SchemaDatasetFacet) schemaFacet).fields()
                                                .stream()
                                                .map(f -> (Object) f)
                                                .collect(java.util.stream.Collectors.toList());
                        }
        ...
        @ResponseStatus(HttpStatus.NO_CONTENT)
        public void deleteTag(@PathVariable String namespace, @PathVariable String datasetName,
