...
// Mock components
vi.mock('@/features/dashboard/components/MiniGraphContainer', () => ({
  MiniGraphContainer: ({ label }: { label: string }) => (
    <div data-testid={`mini-graph-${label}`}>{label}</div>
  ),
}))

vi.mock('@/features/dashboard/components/StackedLineageEvents', () => ({
...
vi.mock('@/features/dashboard/components/JobsDrawer', () => ({
  default: () => <div data-testid='jobs-drawer-content'>Jobs Drawer</div>,
}))

vi.mock('@/features/dashboard/components/SplitButton', () => ({
  default: ({
    options,
    onClick,
    onRefresh,
