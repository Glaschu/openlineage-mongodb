...
export const getJobs = async (
  ...
  let url = `${API_URL}/jobs?limit=${limit}&offset=${offset}&hasLineage=${hasLineage}`
  if (namespace) {
    const encodedNamespace = encodeURIComponent(namespace)
    url = `${API_URL}/namespaces/${encodedNamespace}/jobs?limit=${limit}&offset=${offset}&hasLineage=${hasLineage}`
  }
  if (lastRunStates) {
